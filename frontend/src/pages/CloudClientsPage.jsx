...
import { getCloudClients, updateClient } from '../api/clientService';
...
export default function CloudClientsPage() {
  ...
  const baseColumns = [
    ...
    'ip_address', 'price', 'currency', 'start_date', 'expiry_date',
  ...
  const SORT_STORAGE_KEY = `sortConfig_cloud_clients_${username}`;
  ...
  const fetchClients = async (dynamicKeys = []) => {
    try {
      ...
      const response = await getCloudClients(params);
  ...
  const fetchCustomColumns = async () => {
    try {
      const res = await axios.get('/admin/custom-columns?pageKey=cloud_clients');
  ...
  useEffect(() => {
    if ((17 + dynamicColumns.length) > 0 && !columnInitDone) {
      ...
      const saved = localStorage.getItem('cloud_clients_page');
      if (saved) {
        try {
  ...
  const useResizableColumns = (columnWidths, setColumnWidths) => {
    const startResizing = (index, e) => {
      ...
      const handleMouseMove = (e) => {
        ...
        localStorage.setItem('cloud_clients_page', JSON.stringify(newWidths));
  ...
  const handleRenameColumn = async (col) => {
    const newDbKey = `custom_cloud_clients_${newHeaderLabel.trim().toLowerCase().replace(/\s+/g, '_')}`;
    try {
    ...
    <PageWrapper>
      <div className="p-2">
        <div className={`flex ${isMobile ? 'flex-col items-start gap-2' : 'flex-row justify-between items-center'} mb-2`}>
          <h1 className={`text-xl font-semibold ${dark ? "text-blue-300" : "text-indigo-600"}`}>Cloud Server Clients Table</h1>
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {showDropdown && (
                  <div className={`absolute right-0 mt-2 w-48 rounded-md shadow-lg z-20 p-2 ${dark ? 'bg-gray-800 text-white border border-gray-700' : 'bg-white text-blue-900 border border-gray-200'}`}>
                    <>
                      <button
                        onClick={() => setShowAddClientModal({ open: true, service: 'vps' })}
