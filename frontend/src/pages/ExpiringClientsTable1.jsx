...
export default function NotificationSchedulerPage() {
  ...
  const baseColumns = [
    'select', 'title', 'recipient_type', 'sent_to', 'date_sent', 'time_Sent', 'send_status'
  ];
  ...
  const fetchNotifications = async (dynamicKeys = []) => {
    ...
    try {
      ...
      // };
      let params = new URLSearchParams({ dateFilter, page, limit });
      ...
      const response = await axios.get(`/custom-notifications?${params.toString()}`);
      ...
      const filteredData = data.map(notification => {
        ...
        [...requiredColumns, ...dynamicKeys].forEach(col => {
          ...
          filtered[col] = notification[col];
        });
        filtered.id = notification.id;
        return filtered;
      });
  ...
  const useResizableColumns = (columnWidths, setColumnWidths) => {
    const startResizing = (index, e) => {
      ...
      const handleMouseMove = (e) => {
        ...
        const MIN_WIDTHS = [20, 100, 100, 100, 100, 100, 100, 100]; // per static column index
