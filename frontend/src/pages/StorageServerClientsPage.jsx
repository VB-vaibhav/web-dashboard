...
import { getStorageClients, updateClient } from '../api/clientService';
...
export default function StorageServerClientsPage() {
  ...
  const baseColumns = [
    ...
    'user_address', 'price', 'currency', 'start_date', 'expiry_date',
  ...
  const SORT_STORAGE_KEY = `sortConfig_storage_clients_${username}`;
  ...
  const fetchClients = async (dynamicKeys = []) => {
    try {
      ...
      const response = await getStorageClients(params);
  ...
  const fetchCustomColumns = async () => {
    try {
      const res = await axios.get('/admin/custom-columns?pageKey=storage_clients');
  ...
  useEffect(() => {
    if ((17 + dynamicColumns.length) > 0 && !columnInitDone) {
      const totalCols = 17 + dynamicColumns.length;
      const saved = localStorage.getItem('storage_clients_page');
      if (saved) {
        try {
  ...
  const useResizableColumns = (columnWidths, setColumnWidths) => {
    const startResizing = (index, e) => {
      ...
      const handleMouseMove = (e) => {
        ...
        const MIN_WIDTHS = [20, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100]; // per static column index
        ...
        localStorage.setItem('storage_clients_page', JSON.stringify(newWidths));
  ...
  const handleRenameColumn = async (col) => {
    const newDbKey = `custom_storage_clients_${newHeaderLabel.trim().toLowerCase().replace(/\s+/g, '_')}`;
    try {
    ...
    <PageWrapper>
      <div className="p-2">
        <div className={`flex ${isMobile ? 'flex-col items-start gap-2' : 'flex-row justify-between items-center'} mb-2`}>
          <h1 className={`text-xl font-semibold ${dark ? "text-blue-300" : "text-indigo-600"}`}>Storage Clients Table</h1>
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {showDropdown && (
                  <div className={`absolute right-0 mt-2 w-48 rounded-md shadow-lg z-20 p-2 ${dark ? 'bg-gray-800 text-white border border-gray-700' : 'bg-white text-blue-900 border border-gray-200'}`}>
                    <>
                      <button
                        onClick={() => setShowAddClientModal({ open: true, service: 'storage' })}
