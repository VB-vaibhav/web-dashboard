...
import { bulkRestoreClients, bulkDeleteClients } from '../api/clientService';
import axios from '../api/axios';
import { useTableSearch } from '../hooks/useTableSearch';
import Select from 'react-select';
import { components } from 'react-select';
import { useOutletContext } from 'react-router-dom';
import { getCancelledClients } from '../api/clientService';
...
export default function CancelledClientsTable() {
  ...
  const COLUMN_WIDTHS_KEY = 'cancelled_clients_widths';
  ...
  const baseColumns = ['select', 'client_name', 'service', 'middleman_name', 'price', 'currency', 'expiry_date', 'cancelled_on', 'logical_client_id'];
  ...
  const pageKey = 'cancelled_clients';
  const username = useSelector(state => state.auth.username);
  const SORT_STORAGE_KEY = `sortConfig_cancelled_clients_${username}`;
  ...
  //   const data = await getCancelledClients({
  //     dateFilter,
  //     from: range?.from,
  //     to: range?.to,
  //     page,
  ...
  const fetchClients = async (dynamicKeys = []) => {
    ...
    const response = await getCancelledClients(params);
  ...
  const fetchCustomColumns = async () => {
    try {
      const res = await axios.get('/admin/custom-columns?pageKey=cancelled_clients');
  ...
  //     const text = client[key] ? client[key].toString() : '';
  //     maxWidth = Math.max(maxWidth, text.length * 10 + 20);
  //   });

  //   const newWidths = [...columnWidths];
  //   newWidths[index] = maxWidth;
  //   setColumnWidths(newWidths);
  //   localStorage.setItem(COLUMN_WIDTHS_KEY, JSON.stringify(newWidths));
  // };
  ...
  useEffect(() => {
    localStorage.setItem('cancelled_service_filter', serviceFilter);
  }, [serviceFilter]);

  useEffect(() => {
    const saved = localStorage.getItem('cancelled_service_filter');
  ...
  const handleRenameColumn = async (col) => {
    const newDbKey = `custom_cancelled_clients_${newHeaderLabel.trim().toLowerCase().replace(/\s+/g, '_')}`;
    try {
  ...
  return (
    <PageWrapper>
      <div className="p-2">
                ...
                <table className="table-auto w-full text-sm">
                  <thead className={`sticky top-0 text-slate-400 border-b ${dark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-300"}`}>
                    ...
                    {columns.map((col, index) => {
                      if (col === 'middleman_name' && role === 'middleman') return null;
                      if (col === 'select') {
                        return (
                          <th key={col}
                            className={`relative px-2 py-3 font-semibold border-r group text-left  
        ...
        ${dark ? 'border-gray-700' : 'border-gray-300'}  
        whitespace-nowrap`}>{columnLabels[col]}
                        <div
        ...
        ${dark ? 'border-gray-700' : 'border-gray-300'} text-center whitespace-nowrap`}
                  ...
                  {clients.filter(client =>
                    query.trim() === '' || Object.values(client).some(val => (val || '').toString().toLowerCase().includes(query.toLowerCase()))
                  ).map(client => (
                    <tr key={client.id}>
                      {columns.map(col => {
                        if (col === 'middleman_name' && role === 'middleman') return null;
                        ...
                        if (col === 'expiry_date' || col === 'cancelled_on') {
                          return <td key={col} className="px-2 py-2 text-center">{formatDate(client[col])}</td>;
                        }

                        return <td key={col} className="px-2 py-2 text-center">{client[col]}</td>;
                      })}
                    </tr>
                  ))}
                </tbody> */}
                          ...
                          {columns.map((key, index) => columnVisibility[key] && (
                            ...
                            >
                              {key === 'select' ? (
                                ...
                                >
                                  {key === 'expiry_date' || key === 'cancelled_on' ? (
        ...
        >
          <div onClick={() => autoResizeColumn(contextMenu.columnIndex)}>Auto Resize</div>
          <div onClick={() => cancelSort()}>Cancel Sort</div> */}
