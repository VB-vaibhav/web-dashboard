...
//       <h2 className="text-xl font-semibold mb-2">CerberusClientsPage</h2>
...
import { getCerberusClients, updateClient } from '../api/clientService';
...
export default function CerberusClientsPage() {
  ...
  const [showAddClientModal, setShowAddClientModal] = useState({ open: false, service: '' });
  ...
  const pageKey = 'cerberus_clients';
  ...
  const baseColumns = [
    'select', 'client_name', 'email_or_phone', 'middleman_name', 'plan',
    'instance_no', 'price', 'currency', 'start_date', 'expiry_date', 'jitesh_share', 'queen_share', 'umang_share', 'atlas_jitesh',
  ...
  const filteredColumns = role === 'middleman'
    ? baseColumns.filter(col => col !== 'middleman_name' && col !== 'paid_to' && col !== 'jitesh_share' && col !== 'queen_share' && col !== 'umang_share' && col !== 'atlas_jitesh')
    : baseColumns;

  const requiredColumns = filteredColumns;


  const [searchableColumns, setSearchableColumns] = useState(['select', 'client_name', 'email_or_phone', 'middleman_name', 'plan',
    'instance_no', 'price', 'currency', 'start_date', 'expiry_date', 'jitesh_share', 'queen_share', 'umang_share', 'atlas_jitesh',
  ...
  const columnLabels = {
    ...
    jitesh_share: "Jitesh's share",
    queen_share: "Queen's Share",
    umang_share: "Umang's Share",
    atlas_jitesh: "Atlas Jitesh",
  ...
  const SORT_STORAGE_KEY = `sortConfig_cerberus_clients_${username}`;
  ...
  const fetchClients = async (dynamicKeys = []) => {
    try {
      ...
      const response = await getCerberusClients(params);
  ...
  const fetchCustomColumns = async () => {
    try {
      const res = await axios.get('/admin/custom-columns?pageKey=cerberus_clients');
  ...
  useEffect(() => {
    if ((21 + dynamicColumns.length) > 0 && !columnInitDone) {
      ...
      const saved = localStorage.getItem('cerberus_clients_page');
      if (saved) {
        try {
          ...
          if (parsed.length === totalCols) {
            ...
            setColumnWidths(Array(totalCols).fill(70));
          }
        } catch {
          setColumnWidths(Array(totalCols).fill(70));
        }
  ...
  const useResizableColumns = (columnWidths, setColumnWidths) => {
    const startResizing = (index, e) => {
      ...
      const handleMouseMove = (e) => {
        ...
        const MIN_WIDTHS = [20, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100]; // per static column index
        ...
        localStorage.setItem('cerberus_clients_page', JSON.stringify(newWidths));
  ...
  const handleRenameColumn = async (col) => {
    const newDbKey = `custom_cerberus_clients_${newHeaderLabel.trim().toLowerCase().replace(/\s+/g, '_')}`;
    try {
  ...
  const handleDeleteColumn = async (index) => {
    ...
    try {
      ...
      const saved = localStorage.getItem('cloud_cerberus_page');
      ...
      localStorage.setItem('cloud_cerberus_page', JSON.stringify(newWidths));
  ...
  return (
    <PageWrapper>
      <div className="p-2">
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {showDropdown && (
                  <div className={`absolute right-0 mt-2 w-48 rounded-md shadow-lg z-20 p-2 ${dark ? 'bg-gray-800 text-white border border-gray-700' : 'bg-white text-blue-900 border border-gray-200'}`}>
                    <>
                      <button
                        onClick={() => setShowAddClientModal({ open: true, service: 'cerberus' })}
        ...
        {showAddClientModal.open && (
          <AddClientModal
            onClose={() => setShowAddClientModal({ open: false, service: '' })}
            onClientAdded={() => {
              fetchClients(dynamicKeysRef.current); // reload data
              setShowAddClientModal({ open: false, service: '' });
            }}
            contextService={showAddClientModal.service}
            animate
            dark={dark}
          />
        )}
