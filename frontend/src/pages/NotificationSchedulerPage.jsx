...
export default function NotificationSchedulerPage() {
  ...
  // const actionOptions = [
  //   { value: 'create', label: <div className="flex items-center gap-2"><PlusCircle size={16} /> Create New Notification</div> },
  //   { value: 'edit', label: <div className="flex items-center gap-2"><Pencil size={16} /> Edit Notification</div> },
  //   { value: 'delete', label: <div className="flex items-center gap-2"><Trash2 size={16} /> Delete Notification</div> },
  // ];

  // const actionStyles = {
  //   control: (base) => ({ ...base, minWidth: 220, borderRadius: 12, borderColor: '#e5e7eb' }),
  //   menu: (base) => ({ ...base, zIndex: 50, borderRadius: 12 }),
  ...
  const pageKey = 'notification_scheduler';
  ...
  const baseColumns = [
    'select', 'title', 'recipient_type', 'sent_to', 'date_sent', 'time_sent', 'send_status', 'action'
  ];
  ...
  const requiredColumns = baseColumns;


  const [searchableColumns, setSearchableColumns] = useState(['title', 'recipient_type', 'sent_to', 'date_sent', 'time_sent', 'send_status']);
  ...
  const fetchNotifications = async (dynamicKeys = []) => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ dateFilter, page, limit });

      if (dateFilter === 'custom') {
        const fmt = d => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
        params.append('from', fmt(customRange.startDate));
        params.append('to', fmt(customRange.endDate));
      }

      const res = await axios.get(`/custom-notifications?${params.toString()}`);

      const rows = res.data?.notifications ?? [];
      const totalCount = res.data?.total ?? 0;
      setTotal(totalCount);

      const filteredData = rows.map(n => {
        const out = {};
        [...requiredColumns, ...dynamicKeys].forEach(col => {
          if (col !== 'select') out[col] = col === 'action' ? '' : n[col];
        });
        out.id = n.id;
        return out;
      });
      ...
      validKeys.forEach(k => { if (!(k in current)) current[k] = true; });
      const cleaned = Object.fromEntries(Object.entries(current).filter(([k]) => validKeys.includes(k)));
  ...
  const fetchCustomColumns = async () => {
    try {
      const res = await axios.get('/admin/custom-columns?pageKey=notification_scheduler');
    ...
    const loadAll = async () => {
      try {
        ...
        setSearchableColumns([
          'title', 'recipient_type', 'sent_to', 'date_sent', 'time_sent', 'send_status',
          ...dynamicKeys
        ]);
        await fetchNotifications(dynamicKeys);
      } catch (err) {
  ...
  useEffect(() => {
    fetchNotifications(dynamicKeysRef.current);
  }, [page, limit]);
  ...
  const handleEdit = async (id, key, value) => {
    try {
      ...
      await fetchNotifications(dynamicKeysRef.current);
    } catch (err) {
  ...
  useEffect(() => {
    if ((8 + dynamicColumns.length) > 0 && !columnInitDone) {
      ...
      const saved = localStorage.getItem('notification_page');
      if (saved) {
        try {
  ...
  const useResizableColumns = (columnWidths, setColumnWidths) => {
    const startResizing = (index, e) => {
      ...
      const handleMouseMove = (e) => {
        ...
        const MIN_WIDTHS = [10, 50, 50, 50, 50, 50, 50, 50]; // per static column index

        newWidths[index] = Math.max(startWidth + delta, MIN_WIDTHS[index] || 40);
        ...
        localStorage.setItem('notification_page', JSON.stringify(newWidths));
  ...
  const { query, setQuery, filteredData: textFilteredData } = useTableSearch(notifications, searchableColumns);
  ...
  const handleAddColumn = async () => {
    ...
    try {
      await axios.post('/admin/add-column', {
        pageKey: 'notification_scheduler',
        label: trimmed,
  ...
  const handleRenameColumn = async (col) => {
    const newDbKey = `custom_notification_scheduler_${newHeaderLabel.trim().toLowerCase().replace(/\s+/g, '_')}`;
    try {
  ...
  const openEdit = () => {
    if (selected.length !== 1) {
      setAlertMessage("Please Select only one notification");
      setShowAlert(true);
      return;
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      // simplest: refresh the current page data
      await fetchNotifications(dynamicKeysRef.current);
    } catch (e) {
      console.error(e);
      alert('Delete failed.');
  ...
  // helper: show popup
  const openAlert = (msg) => { setAlertMessage(msg); setShowAlert(true); };

  // const handleBulkDeleteNotifications = async () => {
  //   if (selected.length === 0) {
  //     openAlert("Please select atleast one notification.");
  //     return;
  //   }

  //   const ok = window.confirm(
  //     `Delete ${selected.length} selected notification(s)?\n• Scheduled: will be cancelled\n• Sent: will be removed from this table only`
  //   );
  //   if (!ok) return;

  //   try {
  //     await axios.delete('/custom-notifications/bulk', {
  //       data: { ids: selected }            // axios sends body in DELETE via `data`
  //     });

  //     // Optimistic UI: remove from local table
  //     setNotifications(prev => prev.filter(r => !selected.includes(r.id)));
  //     setSelected([]);

  //     openAlert("Deletion Successful");
  //     // or if you prefer a hard refresh:
  //     // await fetchNotifications(dynamicKeysRef.current);
  //   } catch (err) {
  //     console.error(err);
  //     openAlert("Failed to delete notifications. Please try again.");
  //   }
  // };

  const handleBulkDeleteNotifications = async () => {
    if (selected.length === 0) {
      setAlertMessage("Please select at least one notification.");
      setShowAlert(true);
      return;
    }
    try {
    ...
    <PageWrapper>
      <div className="p-2">
        <div className={`flex ${isMobile ? 'flex-col items-start gap-2' : 'flex-row justify-between items-center'} mb-2`}>
          <h1 className={`text-xl font-semibold ${dark ? "text-blue-300" : "text-indigo-600"}`}>Notification Scheduler</h1>
          ...
          <div className="right-4 top-3 flex items-center gap-2 z-10">
            ...
            {/* {role !== 'middleman' && (
              <div className="relative" ref={dropdownRef}>
                <Select
                  ...
                  onChange={(opt) => {
                    if (!opt) return;
                    if (opt.value === 'create') setShowCreate(true);
                    // wire edit/delete when you implement those
                    setOpenActions(false);
                  }}
                  onMenuOpen={() => setOpenActions(true)}
                  onMenuClose={() => setOpenActions(false)}
                  menuIsOpen={openActions}
                  placeholder={<MoreVertical size={18} />}
                  classNamePrefix="rs"
                  styles={actionStyles}
                />
              </div>
            ...
            {role !== 'middleman' && (
              <div className="relative" ref={dropdownRef}>
                ...
                {showDropdown && (
                  <div className={`absolute right-0 mt-2 w-54 rounded-md shadow-lg z-20 p-2 ${dark ? 'bg-gray-800 text-white border border-gray-700' : 'bg-white text-blue-900 border border-gray-200'}`}>
                    <>
                      <button
                      ...
                      {role !== 'admin' && (
                        <button
                          onClick={() => { setShowDropdown(false); handleBulkDeleteNotifications(); }}
                          className={`w-full flex items-center gap-3 px-3 py-2 text-sm rounded-md
          ${dark ? 'hover:bg-gray-700' : 'hover:bg-indigo-50'}`}>
        ...
        ${dark ? 'border-gray-700' : 'border-gray-300'} text-center whitespace-nowrap`}
                  ...
                  <tbody className={`${dark ? "text-slate-300" : "text-blue-950"}`}>
                    {finalData.length === 0 ? (
                      ...
                      finalData.map((notification, idx) => (
                        <tr key={idx}
                        ...
                        >
                          {requiredColumns.map((key, index) => columnVisibility[key] && (
                            ...
                            //     setEditingCell({ id: idx, key, value: notification[key] });
                            //   }
                            // }}
                            >
                              {key === 'select' ? (
                                ...
                                <div className="flex items-center justify-center gap-2">
                                  <button
                                    onClick={() => openView(notification.id)}
                                ...
                                //       handleEdit(notification.id, key, updated);
                                //       setEditingCell({ id: null, key: null, value: '' });
                                //     }}
                                ...
                                : (
                                  ...
                                  >
                                    {key === 'date_sent' ? (
                                      formatDate(notification[key])
                                    ) : key === 'send_status' ? (
                                      <span className={
                                        notification[key] === 'Failed' ? 'text-red-600 font-semibold capitalize' :
                                          notification[key] === 'Scheduled' ? 'text-yellow-500 font-semibold capitalize' :
                                            notification[key] === 'Sent' ? 'text-green-600 font-semibold capitalize' : ''
                                      }>
                                        {notification[key]}
                                      </span>
                                    ) : notification[key]}
                          ...
                          {dynamicColumns.map((col, index) => {
                            ...
                            return (
                              <td
                                ...
                                onDoubleClick={() => setEditingCell({ id: notification.id, key: col.column_name, value: notification[col.column_name] || '' })}
                              >
                                {editingCell.id === notification.id && editingCell.key === col.column_name ? (
                                  <input
                                    value={editingCell.value}
                                    autoFocus
                                    onChange={(e) => setEditingCell({ ...editingCell, value: e.target.value })}
                                  ...
                                  <div className="whitespace-nowrap overflow-hidden text-ellipsis mx-auto" style={{ maxWidth: columnWidths[i] }}>{notification[col.column_name] || ''}</div>
                                )}
                              </td>
                            );
                          })}
        ...
        {showNotificationModal && (
          <CreateNotificationModal
            ...
            onSuccess={fetchNotifications /* call your existing refresh function */}
            dark={dark}
            animate
          />
        )}
        {showEditModal && (
          <EditNotificationModal
            ...
            onSuccess={() => fetchNotifications(dynamicKeysRef.current)}
            dark={dark}
          />
        )}
        {viewId && (
