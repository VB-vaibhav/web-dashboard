...
import { getVarysClients, updateClient } from '../api/clientService';
...
export default function VarysClientsPage() {
  ...
  const baseColumns = [
    'select', 'client_name', 'email_or_phone', 'middleman_name',
    'accounts_count', 'price', 'currency', 'start_date', 'expiry_date',
  ...
  const SORT_STORAGE_KEY = `sortConfig_varys_clients_${username}`;
  ...
  const fetchClients = async (dynamicKeys = []) => {
    try {
      ...
      const response = await getVarysClients(params);
  ...
  const fetchCustomColumns = async () => {
    try {
      const res = await axios.get('/admin/custom-columns?pageKey=varys_clients');
  ...
  useEffect(() => {
    if ((16 + dynamicColumns.length) > 0 && !columnInitDone) {
      const totalCols = 16 + dynamicColumns.length;
      const saved = localStorage.getItem('varys_clients_page');
      if (saved) {
        try {
  ...
  const useResizableColumns = (columnWidths, setColumnWidths) => {
    const startResizing = (index, e) => {
      ...
      const handleMouseMove = (e) => {
        ...
        const MIN_WIDTHS = [20, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100]; // per static column index
        ...
        localStorage.setItem('varys_clients_page', JSON.stringify(newWidths));
  ...
  const handleRenameColumn = async (col) => {
    const newDbKey = `custom_varys_clients_${newHeaderLabel.trim().toLowerCase().replace(/\s+/g, '_')}`;
    try {
    ...
    <PageWrapper>
      <div className="p-2">
        <div className={`flex ${isMobile ? 'flex-col items-start gap-2' : 'flex-row justify-between items-center'} mb-2`}>
          <h1 className={`text-xl font-semibold ${dark ? "text-blue-300" : "text-indigo-600"}`}>Varys Clients Table</h1>
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {showDropdown && (
                  <div className={`absolute right-0 mt-2 w-48 rounded-md shadow-lg z-20 p-2 ${dark ? 'bg-gray-800 text-white border border-gray-700' : 'bg-white text-blue-900 border border-gray-200'}`}>
                    <>
                      <button
                        onClick={() => setShowAddClientModal({ open: true, service: 'varys' })}
