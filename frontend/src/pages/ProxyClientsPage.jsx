...
import { getProxyClients, updateClient } from '../api/clientService';
...
export default function ProxyClientsPage() {
  ...
  const baseColumns = [
    ...
    , 'proxy_set', 'proxy_count', 'price', 'currency', 'start_date', 'expiry_date',
  ...
  const [searchableColumns, setSearchableColumns] = useState(['client_name', 'email_or_phone', 'middleman_name', 'plan',
    'proxy_set', 'proxy_count', 'price', 'currency', 'start_date', 'expiry_date',
  ...
  const SORT_STORAGE_KEY = `sortConfig_proxy_clients_${username}`;
  ...
  const fetchClients = async (dynamicKeys = []) => {
    try {
      ...
      const response = await getProxyClients(params);
  ...
  const fetchCustomColumns = async () => {
    try {
      const res = await axios.get('/admin/custom-columns?pageKey=proxy_clients');
    ...
    const loadAll = async () => {
      try {
        ...
        setSearchableColumns([
          'client_name', 'email_or_phone', 'middleman_name', 'plan',
          'proxy_set', 'proxy_count', 'price', 'currency', 'start_date', 'expiry_date',
          'middleman_share', 'payment_status', 'amount_paid', 'amount_due', 'paid_to', 'notes',
          ...dynamicKeys
        ]);
  ...
  useEffect(() => {
    if ((18 + dynamicColumns.length) > 0 && !columnInitDone) {
      const totalCols = 18 + dynamicColumns.length;
      const saved = localStorage.getItem('proxy_clients_page');
      if (saved) {
        try {
  ...
  const useResizableColumns = (columnWidths, setColumnWidths) => {
    const startResizing = (index, e) => {
      ...
      const handleMouseMove = (e) => {
        ...
        const MIN_WIDTHS = [20, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100]; // per static column index
        ...
        localStorage.setItem('proxy_clients_page', JSON.stringify(newWidths));
  ...
  const handleRenameColumn = async (col) => {
    const newDbKey = `custom_proxy_clients_${newHeaderLabel.trim().toLowerCase().replace(/\s+/g, '_')}`;
    try {
    ...
    <PageWrapper>
      <div className="p-2">
        <div className={`flex ${isMobile ? 'flex-col items-start gap-2' : 'flex-row justify-between items-center'} mb-2`}>
          <h1 className={`text-xl font-semibold ${dark ? "text-blue-300" : "text-indigo-600"}`}>Proxy Clients Table</h1>
              ...
              {showDropdown && (
                <div className={`absolute right-0 mt-2 w-48 rounded-md shadow-lg z-20 p-2 ${dark ? 'bg-gray-800 text-white border border-gray-700' : 'bg-white text-blue-900 border border-gray-200'}`}>
                  <>
                    {role !== 'middleman' && (
                      <button
                        onClick={() => setShowAddClientModal({ open: true, service: 'proxy' })}
            ...
            >
              ...
              {showSubmenu && (
                ...
                >
                  ...
                  ].filter(col => col?.key && col.key !== 'select' && col.key !== 'logical_client_id')  // optional
                    .map(col => (
                      <button
                        key={col.key}
