// export default function PanelAccessSettings() {
//   return <div>🛠 Panel Access Control Settings</div>;
// }
...
export default function ServiceAccessSettings() {
  ...
  const SORT_STORAGE_KEY = `sortConfig_panel_access_${username}`;
  ...
  const allServiceKeys = [
    { label: 'Notification', key: 'is_notification' },
    { label: 'Mail', key: 'is_mail' },
    { label: 'Reports', key: 'is_reports' },
    { label: 'Settings', key: 'is_settings' }
  ];
  ...
  useEffect(() => {
    if ((7 + dynamicColumns.length) > 0 && !columnInitDone) {
      ...
      const saved = localStorage.getItem('columnWidths_panel_access');
      if (saved) {
        try {
  ...
  const useResizableColumns = (columnWidths, setColumnWidths) => {
    ...
    const startResizing = (index, e) => {
      ...
      const handleMouseMove = (e) => {
        ...
        localStorage.setItem('columnWidths_panel_access', JSON.stringify(newWidths));
  ...
  const fetchUsers = async () => {
    try {
      const res = await axios.get('/admin/panel-access-users');
      ...
      const metaRes = await axios.get('/admin/custom-columns?pageKey=panelAccess');
  ...
  const handleAddColumn = async () => {
    ...
    // const label = trimmed;
    // const safeName = trimmed.replace(/\s+/g, '_').toLowerCase();
    ...
    try {
      ...
      localStorage.setItem('columnWidths_panel_access', JSON.stringify(adjustedWidths));
  ...
  const handleDeleteColumn = async (index) => {
    ...
    try {
      await axios.delete('/admin/delete-column', { data: { columnName: columnToDelete, pageKey: 'panelAccess' } });
  ...
  const getColumnKeyFromIndex = (index) => {
    const staticKeys = ['select', 'name', 'role', 'is_notification', 'is_mail', 'is_reports', 'is_settings'];
      ...
      <div className=" mt-3">
        <div style={{ width: `${totalWidth}px`, minWidth: `100%` }}>
          ...
          <table className="table-auto text-sm w-full ">
            <thead>
              <tr>
                {['select', 'name', 'role', 'is_notification', 'is_mail', 'is_reports', 'is_settings'].map((key, index) => {
        ...
        ${index === 0 ? 'text-left' : 'text-center'} whitespace-nowrap`}
                ...
                /* {['', 'Name', 'Role', 'Cloud Server', 'Cerberus', 'Proxy', 'Storage Server', 'Varys'].map((label, index) => (
                  ...
                  >
                    ...
                    <div onMouseDown={(e) => startResizing(index, e)} className="absolute -right-[1px] top-0 h-full w-1 cursor-col-resize group-hover:bg-indigo-400 z-10" />
                  </th>
              ...
              {filteredData.length === 0 ? (
                ...
                sortedData.map(user => (
                  <tr key={user.id}>
                    ...
                    {['is_notification', 'is_mail', 'is_reports', 'is_settings'].map((key, i) =>
                      columnVisibility[key] && (
            ...
            {showSubmenu && (
              <div
                ...
                {[
                  { key: 'name', label: 'Name' },
                  { key: 'role', label: 'Role' },
                  { key: 'is_notification', label: 'Notification' },
                  { key: 'is_mail', label: 'Mail' },
                  { key: 'is_reports', label: 'Reports' },
                  { key: 'is_settings', label: 'Settings' },
      ...
      {showServiceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className={`rounded-lg p-6 max-w-sm w-80 shadow-lg ${dark ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'}`}>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-semibold">Access to Panels</h2>
