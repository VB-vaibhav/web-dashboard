...
//     [emailOrUsername, emailOrUsername],
//     async (err, results) => {
//       // console.log("Login requested for:", emailOrUsername);
...
exports.login = async (req, res) => {
  ...
  db.query(
    ...
    async (err, results) => {
      ...
      if (user.is_restricted === 1) {
        return res.status(403).json({ error: "You are restricted" });
        // return res.status(403).send('restricted');
      }
...
exports.getMiddlemen = async (req, res) => {
  try {
    const [rows] = await db.query("SELECT DISTINCT username as name FROM users WHERE role = 'middleman'");
...
exports.getAllUsers = async (req, res) => {
  try {
    const [rows] = await db.query("SELECT full_name as name FROM users");
    res.json(rows);
  } catch (err) {
    res.status(500).json({ error: 'Server Error' });
  }
